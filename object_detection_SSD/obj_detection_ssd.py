...
def detect(frame, net, transform):  # We define a detect function that will take as inputs, a frame, a ssd neural network, and a transformation to be applied on the images, and that will return the frame with the detector rectangle.
                                                                         ...
                                                                         #  net - SSD network, transform - transforming the frame to make it compatible with the network    height, width = frame.shape[0], frame.shape[1]                                                                          
    ...
    scale = torch.Tensor([width, height, width, height])                 # We create a tensor object of four dimensions [width, height, width, height(first 2 for upper left and next 2 lower right)]
    ...
    for i in range(detections.size(1)):                                   # detections.size(1) = int(num_of_classes)
        ...
        j = 0                                                              # j will correspond to the occurrences of the clas
    
        while detections[0, i, j, 0] >= 0.6:                               # confidence_score > 0.6 ie all the occurrences j of the class i that have a matching score larger than 0.6
            ...
            cv2.putText(frame, labelmap[i - 1], (int(pt[0]), int(pt[1])), cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 255, 255), 2, cv2.LINE_AA)  #  Putting the Label at upper left point of rectangle 
...
transform = BaseTransform(net.size, (104/256.0, 117/256.0, 123/256.0)) # We create an object of the BaseTransform class, a class that will do the required transformations so that the image can be the input of the neural network.
                                                                       ...
                                                                       # net.size is target size of images that we feed to the network
...
writer = imageio.get_writer('output.mp4', fps = fps) # We create an output video with this same fps

for i, frame in enumerate(reader): # We iterate on the frames of the output video

    frame = detect(frame, net.eval(), transform) # We call our detect function (defined above) to detect the object on the frame, net.eval() as in ssd.py
